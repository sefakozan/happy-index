...
MASK_MODEL_PATH = path.join(CWD, "ai_mask_model", "mask_model.h5")
FACE_MODEL_PATH = path.join(CWD, "face_detector")
THRESHOLD = 0.5
CANVAS_SIZE = 128
...
def detect_and_predict_mask(frame, faceNet, maskNet, threshold):
    ...
    for i in range(0, detections.shape[2]):
        ...
        if confidence > threshold:
            ...
            face = cv2.resize(face, (CANVAS_SIZE, CANVAS_SIZE))
...
while True:
    ...
    for (box, pred) in zip(locs, preds):
        ...
        (correct, incorrect, normal) = pred

        label = "NONE"

        if normal > incorrect and normal > correct:
            label = "Normal"
            color = (255, 0, 0)
        ...
        cv2.rectangle(
            frame,
            (startX, startY + math.floor((endY - startY) / 1.6)),
            (endX, endY),
            color,
            -1,
        )
    ...
    cv2.imshow("Test Mask Model", frame)
